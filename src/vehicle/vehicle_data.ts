...
// Example: interactive prompt (optional)
...
export async function getVehicleData(
    ...
    const records = await fetchVehicleRecords(make, model, model_year);
    if (!records.length) {
        console.error("No records found.");
        return emptyVehicleEntry;
    }

    console.log("\nAvailable vehicles:");
    console.log(
        " # | Make | Model | Year | Cylinders | Engine (L) | Fuel | Transmission | Class"
    );
    console.log("------------------------------------------------------------------");
    records.forEach((rec, i) => {
        console.log(
            `${i + 1} | ${rec.Make ?? "N/A"} | ${rec.Model ?? "N/A"} | ${
                rec["Model year"] ?? "N/A"
            } | ${rec.Cylinders ?? "N/A"} | ${rec["Engine size (L)"] ?? "N/A"} | ${
                rec["Fuel type"] ?? "N/A"
            } | ${rec.Transmission ?? "N/A"} | ${rec["Vehicle class"] ?? "N/A"}`
        );
    });
    console.log("------------------------------------------------------------------");
    ...
    const choice: number = await new Promise((resolve) => {
        rl.question("Select a vehicle (number): ", (answer) => {
            rl.close();
            resolve(parseInt(answer));
        });
    });

    return selectVehicle(records, choice - 1);
}
