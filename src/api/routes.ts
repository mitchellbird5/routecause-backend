...
router.post("/trip", async (req: Request, res: Response) => {
    ...
    if (!body.vehicle_id || !body.make || !body.model || !body.year) {
    ...
    try {
        const vehicles = await fetchVehicleRecords(body.make, body.model, body.year);
        ...
        const trip = await calculateTrip(start, end, vehicle);
